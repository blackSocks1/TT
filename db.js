...
function userTTInit(noCells = 42, initValue = {
  courseInfo: '',
  level: '',
  venue: ""
}) {
...
// sysAdmin.save().then( () =>console.log(`${sysAdmin.name} was successfully saved`));
...
coord.save().then(() => console.log(`${coord.name} was successfully saved`));
...
function fillDb() {
  ...
  for (let i = 0; i < 40; i++) {
    ...
    let lecturer = new Lecturer({
      ...
      TT: [],
      avail_TT: {
        default: userTTInit(42, 'A'),
        currentWeek: []
      },
      temp_TT: userTTInit(42, {
        courseInfo: '',
        level: '',
        venue: ""
      }),
