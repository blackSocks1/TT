...
// export const socket = io.connect(hostLink);

// system classes

!(() => {
  let fx = {
    hostLink: `http://${"localhost"}:${45000}`,
    //   User: class {
    ...
    //             out[1].value = `${
    //               this.accountType === "lecturer" ? period.level : period.lecturerName
    //             }`;
    ...
    //               new Date(period.start).getMinutes() === 0
    //                 ? "00"
    //                 : new Date(period.start).getMinutes()
    //             } - ${
    ...
    //   Coordinator: class extends Lecturer {
    //     constructor(_id) {
    //       super(_id);
    ...
    equalPeriods: function (periodA, periodB) {
    ...
    equalperiodArray: function (Arr, Arr1) {
    ...
    findElement: function (criteria = "_id", value, iterableCollection = [], lastIndex = false) {
    ...
    arrayInit: function (type = "period") {
    ...
    // Other stuffs
    isEquivalent: function (first, second) {
  ...
  //   sysDefaults = await fetch("/admin/getSysDefaults");
