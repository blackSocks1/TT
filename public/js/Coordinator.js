...
export class Coordinator extends Lecturer {
  ...
  coordInit = async () => {
    ...
    this.venueDbClone = [];
    this.groupDb = [];
    this.coordWorker = new Worker("js/coordWorker.js", { type: "module" });
  ...
  setCoordEventListeners = async () => {
    ...
    this.socket.on("/updateAvail", (lecturer) => {
      let lect = fx.findElement("_id", lecturer._id, this.lecturerDb);
      if (lect) {
        this.lecturerDb[lect.index].avail.defaultAvail = lecturer.avail;

        this.lecturerDb[lect.index].avail.weekAvail.forEach((weekAvail) => {
          weekAvail.periods.forEach((availPeriod, index) => {
            availPeriod.state =
              availPeriod.state == "N\\A" &&
              this.lecturerDb[lect.index].avail.defaultAvail[index].state == "A"
                ? "A"
                : availPeriod.state;
          });
        });

        this.lecturerDb[lect.index].availHolder = fx.clone(this.lecturerDb[lect.index].avail);
        if (lecturer._id != this._id) {
          this.showSnackBar(`${lect.element.name}'s availability just updated`);
        }
      }
    });
  };
  ...
  getDbData = async (group_id = "") => {
    let dataLecturers = await fx.dataCom.post("/coord/get-lecturers", {
      _id: this.coordinator_Ref,
    });

    let dataVenues = await fx.dataCom.post("/coord/get-venues", { _id: this._id });

    let dataGroups = await fx.dataCom.post("/coord/get-groups", { _id: this.coordinator_Ref });
    ...
    for (let lecturer of this.lecturerDb) {
      ...
      lecturer.availHolder = lecturer.avail ? fx.clone(lecturer.avail) : {};
    }
    ...
    this.venueDbClone = fx.clone(this.venueDb);
        ...
        if (e.target.id == "avail-lect-names") {
          if (lecturer) {
            ...
            this.consultAvail.periods.forEach((period, index) => {
              if (lecturer.avail.defaultAvail[index]?.state === "A") {
                period.checked = true;
              } else {
  ...
  validate_TT = () => {
    ...
    this.lecturerDb.forEach((lecturer) => fx.resetTempTT(lecturer));
    ...
    this.program_TT.periods.forEach((period, index) => {
      this.TTperiodStartStops[index] = {
        start: Number(period.querySelector("input.start").value),
        stop: Number(period.querySelector("input.stop").value),
      };
    });
    ...
    for (let i = 0; i <= this.sysDefaults.weekDays.length - 1; i++) {
      ...
      while (counter < this.sysDefaults.periods.length) {
        let period = this.program_TT.periods[initIndex].querySelectorAll("input");
        ...
        if (courseInput.disabled) {
          this.TTdisabledPeriods.push(initIndex);

          let oldPeriod = fx.findElement("week", weekToProgram, group.TT);
          if (oldPeriod) {
            oldPeriod = oldPeriod.element.periods[initIndex];
            ...
            group_id = oldPeriod.group_id;
          } else {
            courseName = courseInfo = lecturerName = venue_id = periodState = lecturer_id = group_id =
              "";
          }
        } else if (courseName === "" && lecturerName === "" && venue_id === "") {
          ...
          if (course) {
            ...
            if (course.time.left == 0) {
              ...
              let schedule = {
                index: initIndex,
                dateStart: this.TTperiodStartStops[initIndex].start,
                hoursLeft,
              };
            ...
            if (availCoordinator._id === this._id || availCoordinator._id === "") {
              ...
              fx.setPopupText(
                ...
                `Can't program ${lecturer.name} for this period because ${
                  ...
                  lecturer.avail.weekAvail[weekProgrammed.index].periods[initIndex].coordinator.name
                }).`
              );
          ...
          if (venue) {
            ...
            if (!weekProgrammed) {
              ...
              this.TTperiodStartStops.forEach((period, index) => {
            ...
            let programmedBy = venue.programs[weekProgrammed.index].events[initIndex].programmedBy;
        ...
        this.TTperiods[initIndex] = new Period(
          ...
          periodState,
          lecturer_id,
          this.TTperiodStartStops[initIndex].start,
          this.TTperiodStartStops[initIndex].stop
        );
    ...
    // console.log(this.TTperiods);

    this.coordWorker.postMessage({
      task: "validate timetable",
      data: fx.clone({
        coord: { _id: this._id, name: this.name },
        lecturerDb: fx.clone(this.lecturerDb),
        courseDb: this.courseDb,
        venueDb: this.venueDb,
        TTperiods: fx.clone(this.TTperiods),
        TTperiodStartStops: this.TTperiodStartStops,
        TTprogrammedPeriods: this.TTprogrammedPeriods,
        TTdisabledPeriods: this.TTdisabledPeriods,
        group,
        weekToProgram: fx.clone(weekToProgram),
      }),
    });

    this.coordWorker.onmessage = (e) => {
      if (e.data.task == "validate timetable") {
        let response = e.data.data;
        Object.keys(response).forEach((key) => (this[key] = response[key]));
        console.log(this.TTupdatedCourses, this.TTupdatedLecturers);
        ...
        let empty = emptyTable.includes("false") || this.modPeriods.length > 0; // false --> whole table is empty and true --> not empty
  ...
  send_TT = async () => {
    if (this.TTupdatedLecturers.length > 0) {
      ...
      await fx.dataCom.post("/coord/upGroupTT", {
        group_id: group._id,
        TT: {
          week: weekToProgram,
      ...
      this.TTupdatedCourses.forEach(async (course) => {
        await fx.dataCom.post("/coord/TT-upCourse", { course });
      });

      await fx.dataCom.post("/coord/TT-upLecturers", {
        coord_id: this._id,
        lecturers: this.TTupdatedLecturers,
      });

      this.TTupdatedVenues.forEach(async (venue) => {
        await fx.dataCom.post("/coord/TT-upVenues", {
          week: weekToProgram,
          venue,
        });
      });
  ...
  getCustomClassList = async (group_id = "", columns = "") => {
    return await fx.dataCom.post("/Att/getCustomClassList", { group_id, columns });
  };
